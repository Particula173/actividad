...
def assess_row(row: pd.Series, cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Reputación
    score, reasons = _reputation(rep, cfg, score, reasons)

    # Factores contextuales
    score, reasons = _contextual_risks(row, cfg, score, reasons, rep)

    # Buffer de frecuencia
    score, reasons = _frequency_buffer(row, score, reasons, rep)

    # Decisión final
    return _decision(score, reasons, cfg)


# ======== Helpers ========

def _is_hard_block(row, cfg):
    return (
